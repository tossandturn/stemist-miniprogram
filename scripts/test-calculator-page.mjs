...
test('shift is consumed once and enters the alternate function',()=>{const{page}=setup();page.runAction('shift');page.press({currentTarget:{dataset:{value:'sin(',shiftValue:'asin('}}});assert.equal(page.data.expression,'asin()');assert.equal(page.data.cursor,5);assert.equal(page.data.shiftActive,false)})
