...
for(const route of registered.allPages){
 ...
 if(route===curriculumRoute)page.data={...page.data,board:'ap',boardLabel:'AP',course:'ap-calculus-ab',year:'2025',query:'private-query'}
 ...
 if(route===curriculumRoute){assert.equal(result.query,'board=ap&course=ap-calculus-ab&year=2025');assert.doesNotMatch(result.query,/private-query/)}
}
