...
const r=miniRuntime({
 modules:{'utils/speakingTicket':{issueSpeakingTicket:async()=>{ticketCalls++;return 'realtime-ticket-fixture'}}},
 wx:{getRecorderManager:()=>recorder,createWebAudioContext:()=>audio,authorize:opts=>opts.success(),connectSocket:value=>{socketOptions=value;return socket},setKeepScreenOn(){},base64ToArrayBuffer:()=>new ArrayBuffer(100)}
})
...
await engine.start();assert.equal(ticketCalls,1);assert.equal(socketOptions.header['X-STEMist-Realtime-Ticket'],'realtime-ticket-fixture');socket.open();assert.equal(JSON.parse(sent[0]).type,'connect');assert.equal(JSON.parse(sent[0]).instructions,undefined);assert.ok(JSON.parse(sent[0]).context)
engine.message({type:'event',eventType:'session.updated',payload:{}})
assert.equal(sent.map(value=>typeof value==='string'?JSON.parse(value):null).find(event=>event?.type==='response.create')?.intent,'opening')
...
assert.equal(sent.filter(value=>typeof value==='string'&&JSON.parse(value).intent==='next-question').length,1)
...
assert.equal(sent.filter(value=>typeof value==='string'&&JSON.parse(value).intent==='assessment').length,1)
