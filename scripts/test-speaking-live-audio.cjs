...
async function main(){
 ...
 const clips=(noisy?[0]:[0,1,2]).map(i=>pcm(path.join(directory,'answer-'+i+'.wav')))
 await evaluate('function(){if(getApp().__liveSpeechQa&&!getApp().__liveSpeechQa.finished)throw Error("Speech QA already running");getApp().__speechQaClips='+JSON.stringify(clips.map(()=>''))+';getApp().__speechQaNoisy='+noisy+';return true}')
 ...
 await evaluate(function(){
  ...
  const qa={finished:false,results:[],trace:[],audioByAnswer:{},index:0,phase:'connecting',micUsed:false,studentRecordsWritten:false};app.__liveSpeechQa=qa
  ...
  const done=phase=>{qa.phase=phase;qa.finished=true;clearInterval(interval);clearTimeout(timeout);engine.close();delete app.__speechQaClips;delete app.__speechQaNoisy}
 ...
 const result=await until(function(){const q=getApp().__liveSpeechQa;return q?.finished?{phase:q.phase,results:q.results,trace:q.trace.slice(-45),audioAnswers:Object.keys(q.audioByAnswer).length,audioReceived:q.audioReceived,micUsed:q.micUsed,studentRecordsWritten:q.studentRecordsWritten}:null},'real synthetic speaking dialogue',150000)
 ...
 if(!noisy)for(const turn of answers.slice(-2))assert.doesNotMatch(turn.text,/thank you|meant by|are you asking/i,'Unclear audio gets one neutral restatement, not a guessed interpretation')
}
