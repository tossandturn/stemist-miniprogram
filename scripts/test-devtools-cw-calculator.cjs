...
;(async () => {
  ...
  try {
    ...
    const type=async value=>{await tap('.cw-expression-scroll');await until('.expression-input');await(await p.$('.expression-input')).input(value);await state('expression',value)}
