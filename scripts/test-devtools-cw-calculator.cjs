...
;(async () => {
  ...
  try {
    ...
    const bounded=(promise,ms)=>{let timer;return Promise.race([promise,new Promise((_,reject)=>{timer=setTimeout(()=>reject(new Error('CW UI response timed out after '+steps+' steps')),ms)})]).finally(()=>clearTimeout(timer))}
    const until=async condition=>{const end=Date.now()+10000;while(Date.now()<end){if(await bounded(Promise.resolve().then(()=>typeof condition==='string'?p.$(condition).then(Boolean):condition()),Math.max(1,end-Date.now())))return;await new Promise(r=>setTimeout(r,60))}throw new Error('CW UI state not reached after '+steps+' steps')}
    const tap=async selector=>{await until(async()=>{const e=await p.$(selector);return e&&Number((await e.size()).height)>=44});await(await p.$(selector)).tap()}
    const key=id=>tap(`[data-key="${id}"]`)
    const menu=async id=>{
      const selector=`[data-id="${id}"]`;await until(selector)
      const e=await p.$(selector),box=await p.$('.cw-menu-scroll'),s=await e.size(),o=await e.offset(),bs=await box.size(),bo=await box.offset()
      if(o.top<bo.top||o.top+Number(s.height)>bo.top+Number(bs.height)){
        await box.scrollTo(0,Math.max(0,(Number(await box.property('scrollTop'))||0)+o.top-bo.top))
        await until(async()=>{const eo=await e.offset(),es=await e.size(),so=await box.offset();return eo.top>=so.top-1&&eo.top+Number(es.height)<=so.top+Number(bs.height)+1})
      }
      await tap(selector)
    }
    const state=async (field,value)=>until(async()=>await p.data(field)===value)
    const clear=async()=>{await key('ac');await state('expression','')}
    const type=async value=>{await key('tools');await state('menu','tools');await menu('keyboard');await until('.expression-input');await(await p.$('.expression-input')).input(value);await state('expression',value)}
    const field=async(id,value)=>{await(await p.$(`.cw-field-input[data-id="${id}"]`)).input(String(value));await until(async()=>(await p.data('workFields')).find(f=>f.id===id)?.value===String(value))}
    const log=(name,details={})=>{steps++;console.log(JSON.stringify({name,status:'pass',...details}))}
    await until(async()=>{const e=await p.$('.calc-equals');return e&&Number((await e.size()).height)>0})
    ...
    await clear();await key('fraction');await state('expression','frac(,)');await key('1');await key('down');await key('3');await state('workbench','');await key('exe');await state('hasResult',true);await key('format');await state('menu','format');await menu('format-fraction');await state('display','1/3')
    ...
    await clear();await type('7');await key('exe');await state('answer',7);await key('variable');await state('menu','variables');await menu('variable-A');await state('menu','variable-actions');await menu('store-variable');await until(async()=>(await p.data('variables')).A===7);await key('back');await clear();await key('shift');await state('shiftActive',true);await key('4');await state('expression','A');await key('multiply');await state('expression','A*');await key('2');await state('expression','A*2');await key('exe');await state('answer',14);log('VARIABLE store and SHIFT recall')
    await key('home');await state('menu','home');await menu('app-equation');await state('menu','equation');await menu('work-quadratic');await state('workbench','quadratic');await field('a',1);await field('b',-5);await field('c',6);await tap('.cw-work-submit');await until(async()=>(await p.data('workResults')).length===2);assert.deepEqual((await p.data('workResults')).map(r=>r.value),['2','3']);await tap('.cw-sheet-close');log('HOME Equation with real roots')
    await key('home');await state('menu','home');await menu('app-table');await state('workbench','table');await field('expression','x^2');await field('start',0);await field('end',2);await field('step',1);await tap('.cw-work-submit');await until(async()=>(await p.data('workResults')).length===3);assert.deepEqual((await p.data('workResults')).map(r=>r.value),['0','1','4']);await tap('.cw-sheet-close');log('HOME Table evaluates x for every row')
    await key('home');await state('menu','home');await menu('app-statistics');await state('workbench','statistics');await field('values','1,2,3');await tap('.cw-work-submit');await until(async()=>(await p.data('workResults')).length===7);assert.equal((await p.data('workResults')).find(r=>r.label==='x̄').value,'2');await tap('.cw-sheet-close');log('Statistics mean and standard deviations')
    await key('home');await state('menu','home');await menu('app-base');await state('workbench','base');await field('value','-1');await field('from',10);await field('to',16);await tap('.cw-work-submit');await until(async()=>(await p.data('workResults')).length===1);assert.equal((await p.data('workResults'))[0].value,'FFFFFFFF');await tap('.cw-sheet-close');log('Base-N negative 32-bit two-complement')
    ...
    await clear();await type('1/0');await key('exe');await until(async()=>Boolean(await p.data('error')));assert.equal(await p.data('hasResult'),false);await key('shift');await state('shiftActive',true);await key('ac');await state('powerOff',true);await key('on');await state('powerOff',false);await clear();log('math error, SHIFT OFF and ON')
    assert.equal(faults.length,0,faults.join(';'))
    if(process.env.CW_SCREENSHOT_PATH) {
      await key('fraction');await state('expression','frac(,)');await key('1');await key('down');await key('2');await key('right');await key('plus');await key('fraction');await key('1');await key('down');await key('3');await state('expression','frac(1,2)+frac(1,3)');await key('exe');await state('hasResult',true);await key('format');await state('menu','format');await menu('format-fraction');await state('display','5/6')
