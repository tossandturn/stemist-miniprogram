...
clear();p.append('42');shifted('9');assert.equal(p.data.expression,'42');assert.match(p.data.error,/Complex|复数/,'an unavailable imaginary-unit mode must not silently insert 9')
clear();p.append('42');shifted('x');assert.equal(p.data.expression,'42');assert.match(p.data.error,/QR/,'unsupported QR must not silently insert x')
