...
academic.__catalog={items:Array.from({length:81},(_,i)=>({id:'paper-'+i,subject:'9702',year:2020,file:'paper-'+i+'.pdf',stages:['a2']}))}
academic.applyFilters();assert.equal(academic.data.items.length,30)
academic.loadMore();academic.loadMore();assert.equal(academic.data.items.length,21);assert.equal(academic.data.pageNumber,3)
