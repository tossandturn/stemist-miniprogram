...
async function main(){
 try{
  ...
  await new Promise((resolve,reject)=>execFile(process.execPath,['scripts/test-speaking-live-audio.cjs','--run-production','--audio','D:/CodexWork/stemist-speaking-qa-20260911-0354',...['--room-noise','--long-turn'].filter(flag=>process.argv.includes(flag))],{cwd:process.cwd(),windowsHide:true,timeout:240000,maxBuffer:100000,encoding:'utf8'},(error,stdout,stderr)=>{process.stdout.write(stdout);if(error)reject(Error(stderr.trim()||'Live speech test failed'));else resolve()}))
