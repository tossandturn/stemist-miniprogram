...
for(const page of registered.allPages)assert.doesNotMatch(fs.readFileSync(path.resolve(import.meta.dirname,'..',page+'.wxml'),'utf8'),/<web-view\b/)
for(const feature of r.load('utils/ieltsCatalog').IELTS_FEATURES){assert.equal(feature.kind,'native');assert.ok(registeredPages.has(feature.nativePage.split('?')[0].slice(1)))}
