...
- AI Coach whole-paper marking lives in the ordinary `bundles/marking` subpackage: one answer PDF or ordered images, optional source/mark-scheme PDFs, authenticated asynchronous jobs, resumable per-file uploads, history and PDF reports. Reference-free submissions receive feedback without scores; any supported score is explicitly provisional. See [whole-paper design and verification](docs/whole-paper-marking-plan.md). Backend deployment and real-device acceptance are separate release gates.
