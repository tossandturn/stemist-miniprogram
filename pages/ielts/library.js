...
Page({
 ...
 render(){if(!this.__tasks)return;this.__units=this.data.module==='writing'&&this.data.scope==='pair'?writingPairs(this.__tasks).map(pair=>({...pair,source:'Cambridge IELTS',minutes:60,questions:[],questionCount:0,kind:'pair'})):libraryUnits(this.__tasks,this.data.scope,this.data.topic);const topics=[{key:'',label:'全部主题'},...[...new Map(this.__tasks.flatMap(task=>(task.sections||[]).filter(s=>s.topicKey).map(s=>[s.topicKey,{key:s.topicKey,label:s.topicLabel}]))).values()]];this.setData({...catalogPage(this.__units,this.data),topics})},
