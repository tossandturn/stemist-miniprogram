...
const {readExam,completeExamModule}=require('./nativeExam')
...
function makeObjectivePage(module){return{
 data:deviceState({module,title:module==='listening'?'Listening':'Reading',taskId:'',taskTitle:'',loading:true,busy:false,error:'',questions:[],questionNav:[],current:0,total:0,answer:'',answered:0,sourceImages:[],imageIndex:0,imageCount:0,passageText:'',showPassage:false,audioAvailable:false,audioPlaying:false,audioPosition:0,audioDuration:0,audioTracks:[],audioIndex:0,elapsed:'00:00',saveStatus:'',submitted:false,result:null,review:[]}),
 onLoad(options={}){this.__disposed=false;this.__epoch=epoch();this.__owner=owner();this.__generation=0;this.__examKey=String(options.examKey||'');this.setData({taskId:String(options.taskId||''),examMode:Boolean(this.__examKey)});if(this.__examKey){this.__exam=readExam(this.__examKey);if(!this.__exam||this.__exam.sources[module]!==this.data.taskId){this.setData({loading:false,error:'试题不属于当前模拟。'});return}}if(!this.data.taskId){wx.redirectTo({url:`/pages/ielts/library?module=${module}`});return}this.load()},
 ...
 async load(){const generation=++this.__generation;this.setData({loading:true,error:''});try{
  const task=await getIeltsTask(module,this.data.taskId);if(!this.currentOwner()||generation!==this.__generation)return
  if(!task.questions.length)throw new Error('这份试题尚未完整导入。')
  this.__task=task;this.__storageKey=`stemistIeltsObjective:${this.__owner}:${task.id}`+(this.__examKey?':'+this.__examKey:'')
 ...
 renderQuestion(){
  ...
  this.setData({questions:[q],answer:String(draft.answers[q.id]||''),sourceImages:this.__images[imageIndex]?[this.__images[imageIndex]]:[],imageIndex,imageCount:this.__images.length,
   ...
   questionNav:this.__task.questions.map((item,index)=>({id:item.id,index,label:index+1,current:index===this.data.current,answered:Boolean(String(draft.answers[item.id]||'').trim())}))})
 },
 inputAnswer(event){if(!this.__draft||this.data.submitted||!this.currentOwner())return;const value=String(event.detail.value||'').slice(0,160),q=this.__task.questions[this.data.current];this.__draft.answers[q.id]=value;this.setData({answer:value,answered:Object.values(this.__draft.answers).filter(v=>String(v).trim()).length,saveStatus:'正在保存…',error:''});this.scheduleSave()},
 ...
 elapsedSeconds(){return (Number.isFinite(this.__baseElapsed)?this.__baseElapsed:this.__draft?.elapsed||0)+(this.__activeAt?Math.floor((Date.now()-this.__activeAt)/1000):0)},
 startClock(){clearInterval(this.__clock);if(!this.__draft||this.__draft.submitted)return;this.__baseElapsed=this.__draft.elapsed||0;this.__activeAt=Date.now();this.__clock=setInterval(()=>{if(this.currentOwner())this.setData({elapsed:clock(this.elapsedSeconds())})},1000)},
 ...
 initAudio(){if(!this.data.audioAvailable||!wx.createInnerAudioContext)return;this.__audio=wx.createInnerAudioContext();this.__audio.src=this.__task.audioUrls[this.data.audioIndex];this.__audio.obeyMuteSwitch=false;this.__audio.onPlay(()=>{if(this.currentOwner())this.setData({audioPlaying:true})});this.__audio.onPause(()=>{if(this.currentOwner())this.setData({audioPlaying:false})});this.__audio.onEnded(()=>{if(this.currentOwner())this.setData({audioPlaying:false})});let updated=0;this.__audio.onTimeUpdate(()=>{if(this.currentOwner()&&Date.now()-updated>750){updated=Date.now();this.setData({audioPosition:Math.floor(this.__audio.currentTime||0),audioDuration:Math.floor(this.__audio.duration||0)})}});this.__audio.onError(()=>{if(this.currentOwner())this.setData({audioPlaying:false,error:'音频暂时无法播放，请重试。'})})},
 ...
 selectAudio(event){const index=Number(event.detail.value);if(!this.__audio||!this.__task.audioUrls[index])return;this.__audio.stop();this.__audio.src=this.__task.audioUrls[index];this.setData({audioIndex:index,audioPosition:0,audioPlaying:false})},
 seekAudio(event){this.__audio?.seek(Number(event.detail.value)||0)},
 async submit(){
  ...
  try{
   ...
   this.__draft.submitted=true;this.__draft.result={correct:result.correct,total:result.scoredTotal||result.total,band:result.band,answerAvailable:result.answerAvailable};this.__draft.review=result.details
   ...
   rememberRecord({id:this.__draft.capability.attemptId,taskId:this.data.taskId,title:this.__task.title,skill:module,category:'ielts',band:result.answerAvailable?result.band:null,submittedAt:Date.now(),coachMode:'server'})
