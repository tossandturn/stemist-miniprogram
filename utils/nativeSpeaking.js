...
const {issueSpeakingTicket}=require('./speakingTicket')
...
class NativeSpeaking {
 ...
 async start(){
  ...
  const ticket=await issueSpeakingTicket()
  if(this.closed)return
 ...
 sessionContext(recovery=false){
  return {recovery,task:this.task?{title:this.task.title,part1:this.task.part1,part2:this.task.part2,part3:this.task.part3}:null,completedDialogue:this.turns.slice(-16)}
 }
 async connect(ticket=''){
  if(this.closed)return
  const accessTicket=ticket||await issueSpeakingTicket()
  if(this.closed)return
  ...
  const socket=wx.connectSocket({url:base.replace(/^http/,'ws')+'/qwen-client',header:{'X-STEMist-Realtime-Ticket':accessTicket}})
  ...
  socket.onOpen(()=>{if(!this.closed&&generation===this.generation)this.send({type:'connect',context:this.sessionContext(this.retries>0||this.turns.length>0)})})
