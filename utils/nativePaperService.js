...
async function markPaperChoice(draft,question,selectedOption){
 if(!draft.submitted||!current(draft))throw Error('请先提交试卷。')
 const expected={attemptId:draft.id,mode:'full-paper',routeId:draft.routeId,stage:draft.stage,paperId:draft.paperId,sourceQuestionId:question.sourceQuestionId,selectedOption}
 const result=await gradeObjectiveAnswer(expected)
 if(!current(draft))throw Error('账号已变化，结果未写入。')
 return result
}
...
module.exports={paperSources,paperContext,syncPaperAttempt,markPaperQuestion,markPaperChoice}
