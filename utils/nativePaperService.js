...
async function paperContext(draft){
 ...
 const questions=response.questions.map(q=>{
  ...
  return {number:q.number,sourceQuestionId:q.sourceQuestionId,parts,images:(q.images||[]).filter(url=>/^\/question-assets\/[A-Za-z0-9_-]+\/qp-\d+\.(jpg|jpeg|png|webp)$/.test(url))}
 })
 return {questions}
}
...
module.exports={paperContext,syncPaperAttempt,markPaperQuestion}
